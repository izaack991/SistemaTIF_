...
Public Class frmCAT_Especies
    ...
    Private Sub Guardar(aOperacion As String)
        ...
        Try
            Select Case aOperacion
                Case "A"
                    lMensaje = "¿Esta seguro de dar de alta la nueva especie?"
                Case "M"
                    lMensaje = "¿Esta seguro de modificar el nombre de la especie?"
                    lID = Me.dgvCatalogo.SelectedRows(0).Cells("ID").Value
                Case "C"
                    lMensaje = "¿Esta seguro de cancelar la especie?"
                    lID = Me.dgvCatalogo.SelectedRows(0).Cells("ID").Value
            End Select
            ...
            lCmd.Connection = Cnn
            lCmd.CommandText = "SP_ABC_ESPECIES"
            lCmd.CommandType = CommandType.StoredProcedure

            '
            'DECLARACION DE PARAMETROS EN EL STORED PROCEDURE
            '
            '   PROCEDURE SP_ABC_ESPECIES
            '	@OPERACION  	CHAR(1),
            '	@CODIGO     	INTEGER = NULL,
    ...
    Private Sub Imprimir()
        ...
        Try
            ...
            miReporte = CType(New crptCAT_Especies, ReportDocument)
            If DsCatalogos.Tables("SP_CONS_ESPECIES").Rows.Count > 0 Then
                miReporte.SetDataSource(DsCatalogos.Tables("SP_CONS_ESPECIES"))
    ...
    Private Sub frmCAT_Especies_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.MostrarDatos()
    End Sub
