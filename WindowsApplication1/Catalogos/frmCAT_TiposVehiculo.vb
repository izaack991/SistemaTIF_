...
Public Class frmCAT_TiposVehiculo
    ...
    Private Sub Guardar(aOperacion As String)
        ...
        Try
            Select Case aOperacion
                Case "A"
                    lMensaje = "¿Esta seguro de dar de alta el nuevo tipo de vehiculo?"
                Case "M"
                    lMensaje = "¿Esta seguro de modificar el nombre del tipo de vehiculo?"
                    lID = Me.dgvCatalogo.SelectedRows(0).Cells("ID").Value
                Case "C"
                    lMensaje = "¿Esta seguro de cancelar el tipo de vehiculo?"
                    lID = Me.dgvCatalogo.SelectedRows(0).Cells("ID").Value
            End Select
            ...
            lCmd.Connection = Cnn
            lCmd.CommandText = "SP_ABC_TIPOS_VEHICULO"
            lCmd.CommandType = CommandType.StoredProcedure

            '
            'DECLARACION DE PARAMETROS EN EL STORED PROCEDURE
            '
            '   PROCEDURE SP_ABC_TIPOS_VEHICULO
            '	@OPERACION  	CHAR(1),
            '	@CODIGO     	INTEGER = NULL,
    ...
    Private Sub Imprimir()
        ...
        Try
            ...
            miReporte = CType(New crptCAT_TiposVehiculo, ReportDocument)
            If DsCatalogos.Tables("SP_CONS_TIPOS_VEHICULO").Rows.Count > 0 Then
                miReporte.SetDataSource(DsCatalogos.Tables("SP_CONS_TIPOS_VEHICULO"))
    ...
    Private Sub frmCAT_TiposVehiculo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.MostrarDatos()
    End Sub
