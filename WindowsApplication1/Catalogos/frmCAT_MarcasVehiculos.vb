...
Public Class frmCAT_MarcasVehiculos
    ...
    Private Sub Guardar(aOperacion As String)
        ...
        Try
            Select Case aOperacion
                Case "A"
                    lMensaje = "¿Esta seguro de dar de alta la marca de vehículo?"
                Case "M"
                    lMensaje = "¿Esta seguro de modificar el nombre de la marca de vehículo?"
                    lID = Me.dgvCatalogo.SelectedRows(0).Cells("ID").Value
                Case "C"
                    lMensaje = "¿Esta seguro de cancelar las marca de vehículo?"
                    lID = Me.dgvCatalogo.SelectedRows(0).Cells("ID").Value
            End Select
            ...
            lCmd.Connection = Cnn
            lCmd.CommandText = "SP_ABC_MARCAS_VEHICULOS"
            lCmd.CommandType = CommandType.StoredProcedure

            '
            'DECLARACION DE PARAMETROS EN EL STORED PROCEDURE
            '
            '   PROCEDURE SP_ABC_MARCAS_VEHICULOS
            '	@OPERACION  	CHAR(1),
            '	@CODIGO     	INTEGER = NULL,
    ...
    Private Sub Imprimir()
        ...
        Try
            ...
            miReporte = CType(New crptCAT_MarcasVehiculo, ReportDocument)
            If DsCatalogos.Tables("SP_CONS_MARCAS_VEHICULOS").Rows.Count > 0 Then
                miReporte.SetDataSource(DsCatalogos.Tables("SP_CONS_MARCAS_VEHICULOS"))
        ...
        End Try
    End Sub
    Private Sub frmCAT_MarcasVehiculos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.MostrarDatos()
    End Sub
