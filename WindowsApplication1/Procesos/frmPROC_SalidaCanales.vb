...
Public Class frmPROC_SalidaCanales
    ...
    Dim bTRANSFERENCIA As Boolean
    
    Private Sub frmPROC_SalidaCanales_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        If iPrimeraVez Then
    ...
    Private Sub frmPROC_SalidaCanales_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Limpiar()
        ...
        Me.chksala.Checked = False
        DsSacrificio.SP_CONS_ENTSAL_ENCDET_X_ALMACEN_CANAL.Clear()
        DsSacrificio.SP_CONS_ENTSAL_X_MOVTO_TMP.Clear()
    End Sub
    Public Sub cargarusuario(ByVal iUsuario As Integer, bTRANSFERENCIA As Boolean)
        Me.iUsuario = iUsuario
        Me.bTRANSFERENCIA = bTRANSFERENCIA
        'CARGA EL MOVIMIENTO SALIDA DE CANALES POR EMBARQUE
        If bTRANSFERENCIA = True Then
            oMovAlmacen = New clsMovimientoAlmacen(26)
        ...
        Else
            oMovAlmacen = New clsMovimientoAlmacen(8)
            oMovimientoEntSal = New clsMovimientoEntSal
            oProducto = New clsProductos
    ...
    End Sub
    Private Sub frmPROC_SalidaCanales_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.llenarclasificacion()
    End Sub
    ...
    Private Sub CargaProductos()
        ...
        End Try
    End Sub
    Private Sub llenarclasificacion()
        Dim lCmd As New SqlClient.SqlCommand
        Dim lSql As String
        ...
        With lCmd
            ...
            .CommandText = "Select * From clasificacion_canales"
            .Connection = Cnn
        End With
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        Dim lResp As Integer
        lResp = MessageBox.Show("¿Revisar si canales van a corte o no?", gProyecto, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2)
        If lResp = MsgBoxResult.No Then
            Exit Sub
        Else
            If bTRANSFERENCIA = True Then
    ...
    Private Sub Guardar()
        Try
            ...
            loMovimientoEntSalTMP.SALA_CORTE = chksala.Checked
    ...
    Private Sub Guardar2()
        Try
            ...
            lOperacion = "7"    'SALIDA DE CANALES


            loMovimientoEntSalTMP.OPERACION = lOperacion
            loMovimientoEntSalTMP.ID_MOV = oMovAlmacen.ID_MOV
            loMovimientoEntSalTMP.ID_ALM = oIntroductor.ID_ALM_CAN
            loMovimientoEntSalTMP.ID_ALM_DEST = oDestino.ID_ALM_CAN
            loMovimientoEntSalTMP.ID_CTE_INTR = oIntroductor.ID_CLIENTE
            loMovimientoEntSalTMP.ID_CTE_DEST = oDestino.ID_CLIENTE
            loMovimientoEntSalTMP.FECHA_FRIO = Format(Me.dtpFechaMov.Value, "dd/MM/yyyy")
            loMovimientoEntSalTMP.OBSERVACIONES_FRIO = Me.txtObservaciones.Text.Trim
    ...
    Private Sub DespliegaDatos_TMP()
        Try
            ...
            If Me.bTRANSFERENCIA = True Then
                lSql = "EXECUTE SP_CONS_ENTSAL_X_MOVTO_TMP 26, " & oIntroductor.ID_ALM_CAN.ToString & ", " & oDestino.ID_CLIENTE & "," & "0" & ",0"
            Else
                lSql = "EXECUTE SP_CONS_ENTSAL_X_MOVTO_TMP 8, " & oIntroductor.ID_ALM_CAN.ToString & ", " & oDestino.ID_CLIENTE & "," & "0" & ",0"
            End If
            lCmd.Connection = Cnn
            lCmd.CommandType = CommandType.Text
            lCmd.CommandText = lSql
    ...
    Private Sub AgregarCanal()
        Try
            ...
            loCanalTMP.ID_PED = 0
            loCanalTMP.TEMPERATURA = Me.txttemperatura.Text
            loCanalTMP.ID_CLASIFICACION = Me.cmbclasificacion.SelectedValue
    ...
    Private Sub txtMarcaDestino_TextChanged(sender As Object, e As EventArgs) Handles txtMarcaDestino.TextChanged
