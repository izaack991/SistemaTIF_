...
Public Class frmCONS_EntradaVisceras

    Dim oIntroductor As New clsCliente


    Private Sub frmCONS_EntradaVisceras_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub DespliegaDatos()
        Try
            ...
            If oIntroductor.Existe Then
                lCodAlmacen = oIntroductor.ID_ALM_VIS
            Else
                lCodAlmacen = "%"
            End If

            Me.DsSacrificio.SP_CONS_ENTSAL_VISCERAS_FECHA_ALMACEN.Clear()

            lSql = "SP_CONS_ENTSAL_VISCERAS_FECHA_ALMACEN '" & lCodAlmacen & "', 1, '" & Format(Me.dtpFechaIni.Value, "dd/MM/yyyy") & "', '" & Format(Me.dtpFechaFin.Value, "dd/MM/yyyy") & "'" ', '" & Me.cmbProductos.SelectedValue & "' "
            ...
            da.Fill(Me.DsSacrificio.SP_CONS_ENTSAL_VISCERAS_FECHA_ALMACEN)
    ...
    Private Sub Buscar()
        ...
        Select Case Me.ActiveControl.Name.TrimEnd.ToUpper
            Case "TXTMARCAINTRODUCTOR"
                lsTitulo = "CTE_RELENTVISCERAS_INTRODUCTOR"
